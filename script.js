...
botonEliminar.addEventListener("click", () => {
  localStorage.removeItem("nombres"); // Corregido: cambiar "nombre" por "nombres"
});

//Se pinta de acuerdo a la respuesta dada//
const buttons = document.querySelectorAll(".btn");
let puntos = 0;
let preguntasRespondidas = 0; // Nuevo contador para controlar cuando mostrar el resultado final
const totalPreguntas = document.querySelectorAll(".card").length; // Total de preguntas
...
buttons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    preguntasRespondidas++; // Incrementar contador
    actualizarPuntaje();

    // Mostrar mensaje ganador/perdedor solo cuando se respondan todas las preguntas
    if (preguntasRespondidas === totalPreguntas) {
      const puntosGanador = document.getElementById("puntosGanador");
      if (puntos >= 200) {
        puntosGanador.innerText =
          "Felicitaciones! 🎉 ¡Ganaste! 🎉 2 entradas al teatro!!";
        ...
        puntosGanador.innerText =
          "Estuviste cerca de ganarte unas entradas al teatro 😞 Será la próxima...";
